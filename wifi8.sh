...
while true; do
  ...
  sleep 1
  echo "connection_status is : $connection_status"
  if [ "${connection_status}" = "COMPLETED" ]; then
    echo "Previous network with BSSID $previous_bssid is  still connected"  
  else
    if [ -n "$new_target_bssid" ]; then
      ...
      sleep 8
      r_connection_status=$(wpa_cli -i $interface -p /var/run/wpa_supplicant status | grep -oP 'wpa_state=\K\w+')
      sleep 0.1
      echo "r_connection_status is : $r_connection_status"
    fi    
  fi
