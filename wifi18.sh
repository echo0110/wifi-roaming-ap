...
while true; do
  ...
  iw_signal_dB=$(iw dev wlP2p33s0 link | awk -F 'signal: ' '{print $2}' | cut -d ' ' -f 1)
  ...
  if [ "${connection_status}" = "COMPLETED" ]; then
    if [ "$iw_signal_dB" -lt -67 ]; then
      ...
      #If the connection status is COMPLETED and the signal strength is greater than or equal to -70, execute the logic here"
  ...
  sleep 5
  ifconfig $interface 192.168.108.233 netmask 255.255.0.0
